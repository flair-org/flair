...
def _is_merge_eligible_commit(commit_data: dict[str, Any], commit_dir: Path) -> bool:
    ...
    if commit_data.get("commitType") in {"MERGE"}:
        return False
...
def _find_last_merge_cursor(commits: list[tuple[dict[str, Any], Path]]) -> str:
    for commit_data, _ in reversed(commits):
        if commit_data.get("commitType") == "MERGE":
...
def _aggregate_fedavg(models: list[dict[str, Any]], weights: list[float]) -> dict[str, np.ndarray]:
    if not models:
...
def create_merge_candidate(
    ...
    try:
        ...
        aggregated = _aggregate_fedavg(models, weights)
