...
Flair can sign commits with a dedicated SSH Ed25519 key instead of the Solana wallet keypair flow.
...
- `flair auth ssh setup` generates or registers a Flair SSH key, stores metadata in `~/.flair/ssh.json`, and records the commit principal as `ssh:SHA256:<fingerprint>`.
- `flair auth ssh env` prints or writes a shell activation snippet that sets `FLAIR_SSH_KEY_PATH`, `FLAIR_SSH_KEY_PASSPHRASE`, and `SSH_ASKPASS_PASSWORD` for the current shell session.
- `flair auth ssh status` shows whether the SSH key, metadata, env vars, and current session principal are ready for commit signing.
...
If you protect the key with a passphrase, the CLI reads it from either `FLAIR_SSH_KEY_PASSPHRASE` or `SSH_ASKPASS_PASSWORD`. The activation snippet generated by `flair auth ssh env` prompts for the passphrase instead of storing it in the repo.
